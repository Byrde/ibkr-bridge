...
| **IBKR Authentication Management** | Implement automatic re-authentication | Session expiry triggers re-authentication; re-auth uses stored credentials + TOTP; in-flight requests wait or retry. | `Ready to Test` |  | Critical for long-running deployments. Should leverage existing HeadlessLoginService. |
